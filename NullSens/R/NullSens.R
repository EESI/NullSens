NullSens <-
function(CDM, X, select = TRUE, reg_method="robust", null_reps=200, test_stat = c(1,1), mutual_reject=8, alpha=0.05) {
...
# null_reps = 200 (default) -- number of random matrices to generate for null distribution
...
# mutual_reject = 8 (default) -- ignore pairs with less than 7 mutual sites in testStatistic
...
# IND_summary -- abiotic, biotic, unexplained variation, per species
# COM_summary -- abiotic, biotic, unexplained variation, community
# COM_var_type -- [avg_covar, p-value_pos, p_value_neg]
...
if (select) sites_sel <- sitesSelect(CDM,X) # Select sites
else sites_sel <- list(rep(list(rep(TRUE,n)),p)) # Include all sites

# CHOOSE MULTIVARIATE REGRESSION METHOD
if (reg_method == "standard") mvr_out <- mvrStandard(CDM,X,sites_sel)
else if (reg_method == "robust") mvr_out <- mvrRobust(CDM,X,sites_sel)
else if (reg_method == "tobit") mvr_out <- mvrTobit(CDM,X,sites_sel)
...
for (i in 1:null_reps-1){
	...
	tS_out <- testStatistic(rand_matrix,sites_sel,test_stat,mutual_reject)
	index[i] <- tS_out$index
	cT_out <- covStatistic(rand_matrix,sites_sel,mutual_reject)
	cTindex[i] <- cT_out
}
# Index Computed on Residuals of CDM under Test
tS_out <- testStatistic(mvr_out$Yres,sites_sel,test_stat,mutual_reject)
...
COM_var_type <- list("avg_covar" = cTindex[null_reps], "p_value_pos" = p_value_pos, "p_value_neg" = p_value_neg)

# VARIATION PARTITIONING
# Partition variation -- abiotic, biotic, unexplained
var_expl_out <- varExplained(CDM,p_value,sites_sel,CR,coeff_out$Avg_Adj_R2,coeff_out$Adj_R2,alpha)

#####################################################################################

result = list('CDM'=CDM,'X'=X,'Yhat'=mvr_out$Yhat,'Yres'=mvr_out$Yres,'B_est'=mvr_out$B_est,'sites_sel'=sites_sel,'p_value'=p_value,'test_indices'=index,'CR'=CR,'CR'=CV,'R2'=coeff_out$R2,'Adj_R2'=coeff_out$Adj_R2, 'Avg_R2'=coeff_out$Avg_R2, 'Avg_Adj_R2'=coeff_out$Avg_Adj_R2, 'IND_summary' = var_expl_out$IND_summary, 'COM_summary' = var_expl_out$COM_summary, 'COM_var_type' = COM_var_type)
}
